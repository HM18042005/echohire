...
async def create_ai_guided_interview(
    ...
    try:
        ...
        return AIGuidedInterviewResponse(
            ...
            callId=call_id,
            status=call_status,
            assistantId=vapi_call_result.get("assistantId") if 'vapi_call_result' in locals() and vapi_call_result else "assistant_unavailable",
            publicKey=vapi_call_result.get("publicKey") if 'vapi_call_result' in locals() and vapi_call_result else os.getenv("VAPI_PUBLIC_KEY"),
            workflowId=UNIVERSAL_WORKFLOW_ID,
            interviewId=interview_id,
            message=f"AI guided interview session created for {request.companyName}. Note: Vapi integration failed but interview record created."
        )
